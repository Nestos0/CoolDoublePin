...
var doublePinyinLingYue = schema{
	name:            "cn_en_lingyue",
	desc:            "泠月双拼",
	combinationType: "unique",
	path:            filepath.Join(RimeDir, "en_dicts/cn_en_lingyue.txt"),
	mapping: map[string]string{
		// 零声母
		"-a-":   "oa",
		"-e-":   "oe",
...
func convertToDoublePinyin(code string, s schema) string {
    ...
    validInitials := mapset.NewSet( "a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z", "zh", "sh", "ch" )
	// 声母转换
    if validInitials.Contains(initial) {
        if mappedValue, ok := s.mapping[initial]; ok {
            initial = mappedValue
        }
    }
	// 韵母转换
